...
- **Hero proof point:** side-by-side App Privacy labels — TaxTrail **"Data Not Linked to You — Purchases"** vs Keeper/QuickBooks/Wave advertising disclosures. Use it in screenshot 2 of the listing. **Build the screenshot from the real label, not from "Data Not Collected"** — that is not what TaxTrail ships (D-022), Apple reviews screenshots, and a false label claim in store marketing is a rejection the pitch cannot afford. The true version is still the whole pitch: one unlinked purchase row against pages of identity-linked financial data.
