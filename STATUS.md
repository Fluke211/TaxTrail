...
**Last updated:** 2026-09-14 · Update this file at the end of every working session.
...
| iOS app (`mobile/`) | **v1.0.0 (build 7) · js r39 next; r38 live** | Build 6 opens on Tyler's phone with real icons, which confirms the expo-font pin (D-072). It is the first binary whose **embedded** bundle runs, so the first that is submittable. Build 6 is in `LIVE_BUILDS` (D-074). r30 (live 2026-09-02, group `859ae2fc`) fixed receipt photographs, which stopped rendering because iOS moved the app's Data container (D-075). **r31 confirmed on device 2026-09-03** (live, group `c4c3357e`): the Face ID lock works, and the app's own palette switches to Light over the air on build 6. Whether the window override reaches the SYSTEM surfaces is still unverified (D-081). **r33 published 2026-09-05** (live, group `27cf5272`), carrying r32 and r33 together: the type scale raised twice (D-080, then D-082, to "readable without young eyes"), and light mode's surfaces rebuilt after they measured 1.07:1 between a card and the page under it (D-083). **Confirmed on device 2026-09-05:** the type sizes and the light palette both read well, and the capture screen fits an iPhone 14, though barely. **r34** (live 2026-09-06, group `a7f13686`) gives it another 24 points by moving the privacy elaboration into the ON-DEVICE badge, which is tappable now (D-084). **Build 7 built and submitted to TestFlight 2026-09-06** (EAS `853275f8`, from `88a5061`): it drops `expo-location`, which closes D-066 and was the last thing blocking submission on its own (D-085). It is in `LIVE_BUILDS`, which cleared the publish guard, so OTA updates work again. Apple finished processing it the same day: **build 7 reads VALID and installable** in App Store Connect. **Build 7 confirmed launching on device 2026-09-07.** Monthly and annual sandbox purchases both work. **r35 is the first fix round driven by real receipts**: Tyler scanned 24 and sent the diagnostics, which found sales tax wrong or missing on 12 of 18, the total wrong on 3, and the merchant wrong on 4 (D-087, D-088, D-089). All but one are fixed, and that one was photographed upside down. The corpus tripled to 30 real receipts at zero expectation mismatches. **r35 published 2026-09-14** (live, group `5723d853`). Device feedback on it found that the app learned every parser guess as though it were a correction, so no parser improvement could reach a store already scanned; **r36** fixes that, adds duplicate detection and keeps the user on Capture after a save (D-091). **r36 published 2026-09-14.** **r37** closes the gap D-091 named: a remembered name can now be forgotten from the merchant field, and a correction replaces the old entry instead of stacking beside it. It also asks what a multi-page scan is rather than fusing two receipts, lets a refund be entered by hand (iOS decimal-pad has no minus key), stops dropping a refund's sales tax, keeps the keyboard off the notes field and levels the TOTAL and SALES TAX boxes (D-092). **r37 published 2026-09-14** (group `6e3b8676`). Tyler then sent the full 28-receipt archive with images and OCR, including **four rescans of slips already in the corpus**, which is what exposed the Safeway totals report he could not describe: a coupon-savings block was being read as the grand total ($0.50 for a $48.42 receipt), and an award banner whose year OCR mangled was being read as the merchant. Both fixed in **r38**; corpus grown with the two defects pinned as failures first (D-093). **r38 shipped one of those fixes wrong.** Widening the award-banner filter to "best" plus any four digits also swallowed real names like "America's Best 4412", a national chain, losing the merchant on every one of its receipts; the probes only ever put "best" first, never last before the number. A parallel agent review caught it after r38 was already published. Reverted in **r39**, which also corrects the corpus from a claimed 37 receipts to 31 distinct ones (six were duplicate texts) and adds a gating `test:corpus` so that cannot recur (D-094). The Hele rescan merchant is a known failure again, pinned as one and red in every score run. |
